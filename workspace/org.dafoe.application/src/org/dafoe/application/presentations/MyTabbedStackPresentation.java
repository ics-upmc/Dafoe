...
public class MyTabbedStackPresentation extends StackPresentation {
	private PresentablePartFolder folder;
	private ISystemMenu systemMenu;
	private ISystemMenu partList;
	private PreferenceStoreAdapter apiPreferences = new PreferenceStoreAdapter(
	...
	private TabFolderListener tabFolderListener = new TabFolderListener() {
		public void handleEvent(TabFolderEvent e) {
			...
			case TabFolderEvent.EVENT_PANE_MENU: {
				...
				MyTabbedStackPresentation.this.showPaneMenu(folder
			...
			case TabFolderEvent.EVENT_PREFERRED_SIZE: {
				IPresentablePart part = folder.getPartForTab(e.tab);
				if (part == null) {
					// Standalone views with no title have no tab, so just get
					// the part.
	...
	public MyTabbedStackPresentation(IStackPresentationSite site,
			AbstractTabFolder widget, ISystemMenu systemMenu) {
		this(site, new PresentablePartFolder(widget), systemMenu);
	}

	public MyTabbedStackPresentation(IStackPresentationSite site,
			PresentablePartFolder folder, ISystemMenu systemMenu) {
		this(site, folder, new LeftToRightTabOrder(folder),
				new ReplaceDragHandler(folder.getTabFolder()), systemMenu);
	}

	public MyTabbedStackPresentation(IStackPresentationSite site,
			PresentablePartFolder newFolder, TabOrder tabs,
			TabDragHandler dragBehavior, ISystemMenu systemMenu) {
		super(site);
		...
		folder.getTabFolder().getControl().addDisposeListener(
				new DisposeListener() {
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.presentations.StackPresentation#saveState(org.eclipse.
	 * ui.presentations.IPresentationSerializer, org.eclipse.ui.IMemento)
	 */
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.presentations.StackPresentation#setBounds(org.eclipse.
	...
	/**
	 * Returns the minimum size for this stack, taking into account the
	 * available perpendicular space.
	 * 
	 * @param width
	 *            indicates whether a width (=true) or a height (=false) is
	 *            being computed
	 * @param availablePerpendicular
	 *            available space perpendicular to the direction being measured
	 *            or INFINITE if unbounded (pixels).
	 * @return returns the preferred minimum size (pixels). This is a width if
	 *         width == true or a height if width == false.
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.ui.ISizeProvider#computePreferredSize(boolean, int, int,
	 * int)
	 */
	public int computePreferredSize(boolean width, int availableParallel,
		...
		if (parts.length == 1
			...
			int minSize = computePreferredMinimumSize(width,
					availablePerpendicular);
			if (width) {
				// PaneFolder adds some bogus tab spacing, so just find the
				// maximum width.
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.presentations.StackPresentation#addPart(org.eclipse.ui
	 * .presentations.IPresentablePart, java.lang.Object)
	 */
	public void addPart(IPresentablePart newPart, Object cookie) {
		...
		if (tabs.getPartList().length == 1) {
			if (newPart.getSizeFlags(true) != 0
					|| newPart.getSizeFlags(false) != 0) {
				getSite().flushLayout();
			}
		}
		...
		PresentablePart toto = (PresentablePart) newPart;
		if (toto != null) {
			...
			Color currentColor = themeManager.getTheme(le_theme).getColorRegistry().get("org.dafoe.application.backcolor");
			Color foreColor = themeManager.getTheme(le_theme).getColorRegistry().get("org.dafoe.application.forecolor");
			
			
			MyDefaultTabFolder current = (MyDefaultTabFolder) this.folder
			.getTabFolder();
			
			
			current.updateColors(currentColor,foreColor);
			
			//current.updateColors(currentColor);
			
			/*
			Color[] tabColor = new Color[2];
			int[] pour = new int[1];
			tabColor[0]  = currentColor;
			tabColor[1]  = Display.getDefault().getSystemColor(SWT.COLOR_RED);
			pour[0] = 90;
			
			DefaultTabFolderColors DTFC = new DefaultTabFolderColors(
					Display.getDefault().getSystemColor(SWT.COLOR_WHITE),
					tabColor,pour,true);
			
			
			current.setColors(DTFC, 0);*/
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.presentations.StackPresentation#movePart(org.eclipse.ui
	 * .presentations.IPresentablePart, java.lang.Object)
	 */
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.presentations.StackPresentation#removePart(org.eclipse
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.presentations.StackPresentation#selectPart(org.eclipse
	 * .ui.presentations.IPresentablePart)
	 */
...
//	Color currentColor = new Color(Display.getCurrent(),0,0,255);
	...
	PresentablePart toto = (PresentablePart) toSelect;
		if (toto != null) {
			WorkbenchPartReference coco = (WorkbenchPartReference) toto
					.getPane().getPartReference();
			IWorkbenchPart part = coco.getPart(false);
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.presentations.StackPresentation#dragOver(org.eclipse.swt
	...
	public void showSystemMenu() {
		showSystemMenu(folder.getTabFolder().getSystemMenuLocation(), getSite()
				.getSelectedPart());
	}

	public void showSystemMenu(Point displayCoordinates,
			IPresentablePart context) {
	 ...
	 * 
	 * @see
	 * org.eclipse.ui.presentations.StackPresentation#getTabList(org.eclipse
	 * .ui.presentations.IPresentablePart)
	 */
