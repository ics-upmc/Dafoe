 ...
 *
 */
public class AutocompleteSelectorContentProposalProvider extends
	 ...
	 * the specified array of Objects. This ContentProposalProvider will SELECT
	 * a completion for the input meaning that the text input MUST match one of
	 * the proposed completions
	 * 
	 * @param proposals
	 *            the array of Strings to be returned whenever proposals are
	 *            requested.
	 */
	public AutocompleteSelectorContentProposalProvider(
			List<ITerminoConcept> proposals, Text text) {
		super(proposals);
		this.control = text;
	}
	...
	/**
	 * Return the matching proposals for the text currently in the Text widget.
	 * During the process check that the entered text matches at least one of
	 * the proposals, if not remove the last character entered
	 * 
	 * @param combo
	 *            the widget containing the text
	 * @return list of possible Objects that complete the text contents
	 */
	@SuppressWarnings("unchecked")
	private List getContentProposals(Text text) {
		text.setText(text.getText().substring(0, text.getText().length() - 1));
		text.setSelection(new Point(text.getText().length(), text.getText()
				.length()));
