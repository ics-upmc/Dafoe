...
public class DisplayStatusElements extends AbstractHandler {

	public Object execute(ExecutionEvent event) throws ExecutionException {
		String selectedStatus = event.getParameter("org.dafoe.terminologiclevel.termCard.statusDisplay");  //$NON-NLS-1$
		...
		if (relationViewPart != null) {
			...
			if (selectedStatus.compareTo("all") == 0){ //$NON-NLS-1$
				
				((TerminologicRelationAreaViewPart)relationViewPart).removeFilters();
				
			} else {

				((TerminologicRelationAreaViewPart)relationViewPart).createStatusFilter(status);
