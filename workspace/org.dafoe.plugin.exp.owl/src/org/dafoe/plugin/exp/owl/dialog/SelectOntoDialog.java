...
import org.dafoe.plugin.exp.owl.adaptater.Services;
...
public class SelectOntoDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		setMessage(Messages.SelectOntoDialog_0); 
		setTitle(Messages.SelectOntoDialog_1); 
		...
		ontoListviewer.setInput(Services.getAllOntologies());
		...
		gd.horizontalSpan = 1;
		ontoListviewer.getControl().setLayoutData(gd);
		...
		if (selection != null) {
			if (selection instanceof IStructuredSelection) {
				...
				if (!ss.isEmpty()) {
					...
					if (selo instanceof IOntology) {
						IOntology curOnto = (IOntology) selo;
						ControlerFactory.getControler()
								.setCurrentSelectedOntology(curOnto);
