...
/*	
	public static SessionFactory getDynamicSessionFactory(IDataSource dbSource) {
		...
		config.setProperty("hibernate.connection.url", "jdbc:postgresql:"+dbSource.getDbLocation());
		config.setProperty("hibernate.connection.username", dbSource.getUserName());
		config.setProperty("hibernate.connection.password", dbSource.getUserName());
		sessionFactory = config.buildSessionFactory();
