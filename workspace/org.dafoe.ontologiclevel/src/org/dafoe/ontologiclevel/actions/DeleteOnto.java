...
public class DeleteOnto implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		...
		dial.message = Messages.getString("DeleteOntho.ChoixOntho"); //$NON-NLS-1$
		dial.open();
		
		IOntology onto = dial.getOntology();

		if (onto!=null) {
			MessageBox messageBox = new MessageBox(Activator.getDefault().getWorkbench().getDisplay().getActiveShell(), SWT.ICON_QUESTION|SWT.OK|SWT.CANCEL);
			messageBox.setMessage(Messages.getString("DeleteOntho.EstVousSur")+onto.getName()); //$NON-NLS-1$
			int res = messageBox.open();

			if (res== SWT.OK) {
				org.dafoe.ontologiclevel.common.DatabaseAdapter.deleteOntology(onto);
				...
				Activator.setCurrentOntology(org.dafoe.ontologiclevel.common.DatabaseAdapter.loadDefaultOntology());
			}
		}
	}
