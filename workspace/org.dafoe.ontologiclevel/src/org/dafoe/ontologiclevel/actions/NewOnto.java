...
import org.dafoe.ontologiclevel.Dialog.NewOntology;
...
public class NewOnto implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		/*MessageBox msg = new MessageBox(Activator.getDefault().getWorkbench().getDisplay().getActiveShell(),SWT.ICON_QUESTION | SWT.OK | SWT.CANCEL);
		msg.setMessage("Nouvelle ontologie");
		msg.setText("Voulez-vous créer une nouvelle ontology");
		if (msg.open()==1) {
		...
		NewOntology dial = new NewOntology(Activator.getDefault().getWorkbench().getDisplay().getActiveShell());
		dial.open();
		...
		if (onto!=null) {
			...
			PlatformUI.getWorkbench().getActiveWorkbenchWindow()
					.getActivePage().setPerspective(perspectiveWithId);
			org.dafoe.ontologiclevel.common.DatabaseAdapter.saveOntology(onto);
			Activator.setCurrentOntology(onto);
		}
